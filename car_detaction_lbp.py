...
# 加载特征提取算法
pca = joblib.load('lbp_pca.pkl')

# 加载预训练的AdaBoost分类器
classifier = joblib.load('lbp_pca_adaboost.pkl')
...
for level, resized in enumerate(pyramid_gaussian(gray, downscale=scale_factor,max_layer=levels)): 
    ...
    for (x, y, window) in sliding_window(resized, stepSize, windowSize=(winW, winH)):
        ...
        # 从窗口中提取HOG特征
        features = getLBPH(src=window,P_num=8,Pattern='uniform',grid_x=8,grid_y=8,normed=True)
